...
chunk = pd.read_csv('C:/Users/Scian/Desktop/CH_010716_Voice.txt',
                  encoding = 'mbcs', 
                  iterator = True,
                  chunksize = 1000)
                  #delimiter = ';',
...
bar_plot_nulls(cdr,'Null Features','Null Count', 'Voice' +' Features With Nulls. ')
print(cdr.shape)
cdr = drop_all_null_cols(cdr) #drops the columns where all values are null
...
cdr = cdr.drop(['EVENT_PARAMETER14','EVENT_PARAMETER16','EVENT_PARAMETER25'], axis =1)
cdr = cdr.fillna({'EVENT_PARAMETER7':'35600000000','EVENT_PARAMETER6' : 0})
cdr['EVENT_PARAMETER24'] = cdr['EVENT_PARAMETER24'].fillna(method = 'ffill')
cdr.loc[(cdr['CLEAR_CAUSE'] == 449) , ['DESTINATION']] ='CALL_FAILED_DUE_TO_NO_TARIFF'
#print()
#print('-----------------------------------------------')
#print()
cdr=remove_low_variance(cdr)
new_voice = cdr
